...
class PeriodReportManager extends ReportManager
{
    ...
    public function setPeriod(TimetablePeriod $period): PeriodReportManager
    {
        ...
        foreach($this->getActivities() as $activity)
            $this->addAllocatedStudents($activity);
     ...
     * @return PeriodReportManager
     */
    public function addAllocatedStudents(?ActivityReportManager $act): PeriodReportManager
    {
        if (empty($act))
            return $this;

        foreach ($act->getActivity()->getActivity()->getStudents() as $student) {
            $grade = $student->getStudent()->getStudentCurrentGrade($this->getCalendar());
            $students = $this->getAllocatedStudentGrade($grade);
